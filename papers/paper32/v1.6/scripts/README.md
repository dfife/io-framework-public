...
- `01_compute_framework_constants.py`
  - Recomputes `Q`, `K_gauge`, `f_Gamma`, `Delta`, `R_U`,
    `R4_FIRAS`, `T_obs(R4_FIRAS)`, the historical `T_obs(R4=1)`
    diagnostic, `tau_eff`, `n_s`, and `x_crit`.
...
- `05_kb7_source_block_validation.py`
  - Records the scoped KB.7/P4 source-block validation gates and checks
    `Z(e^x)=Q`. This does not derive Paper 17 optical R4.
...
- `09_r4_firas_impact_audit.py`
  - Reproduces the R4/FIRAS numerical impact and script/manuscript damage
    ledger for the v1.6 repair.
- `10_validate_expected_outputs.py`
