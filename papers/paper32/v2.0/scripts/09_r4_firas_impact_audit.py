#!/usr/bin/env python3
"""
Paper 32 v2.0 reproducibility script 09.

Purpose:
    Generate the R4/FIRAS impact ledger for the Paper 32 v2.0 repair.
...
Claim boundary:
    ...
    are in reports/paper32_v20_r4_firas_kappa_audit_report.md.
"""
    ...
    script_impacts = [
        {
            ...
            "reason": "v1.5 computed T_obs = T_IO*x^K_gauge as active observed CMB value; v2.0 computes T_obs(R4_FIRAS) and keeps R4=1 only as historical diagnostic."
        },
        {
    ...
    manuscript_damage_locations = [
        {
            ...
            "v2_0_action": "Retire independent T_CMB prediction wording; state T_IO is derived and observed CMB is FIRAS-normalized through Paper 17 v1.5."
        },
        {
            "source": "Paper 32 v1.5 extracted text",
            "paragraph": 350,
            "damage": "States T_obs = T_IO*x^K_gauge = 2.7253 K as active formula.",
            "v2_0_action": "Replace with T_obs(R4)=T_IO*x^(R4*K_gauge), R4_FIRAS=1.0031014644105183."
        },
        {
            "source": "Paper 32 v1.5 extracted text",
            "paragraph": 442,
            "damage": "Code/Data section points to v1.5 bundle and unqualified T_obs reproduction.",
            "v2_0_action": "Update to v2.0 bundle and describe T_obs as FIRAS-fixed readout."
        },
        {
            "source": "Paper 32 v1.5 extracted text",
            "paragraph": 446,
            "damage": "Open Problems list merges R4/P4 normalization and marks it closed by KB7.",
            "v2_0_action": "Split P4 source-block closure from Paper 17 optical R4. P4 remains DERIVED/CONDITIONAL_VERIFIED by KB7; R4 is FIRAS-fixed by Paper 17 v1.5."
        },
        {
            "source": "Paper 32 v1.5 extracted text",
            "paragraph": 703,
            "damage": "States T_obs = T_IO*exp(K_gauge/2) = T_IO*sqrt(1+gamma^2) = 2.7253 K.",
            "v2_0_action": "Retire as erroneous/historical; use Paper 17 v1.5 readout family."
        },
        {
            "source": "Paper 32 v1.5 extracted text",
            "paragraph": 704,
            "damage": "Temperature Transfer Theorem says structural pieces force T_obs = T_IO*x^K_gauge.",
            "v2_0_action": "Downgrade to forcing the readout family and gauge payload; R4 fixed separately by FIRAS."
        },
        {
            "source": "Paper 32 v1.5 extracted text",
            "paragraphs": [713, 715, 791],
            "damage": "Uses ln(T_obs/T_IO)=K_gauge*ln x or equivalent.",
            "v2_0_action": "Replace with ln(T_obs/T_IO)=R4*K_gauge*ln x for the optical readout family."
        },
        {
            "source": "Paper 32 v1.5 extracted text",
            "paragraph": 717,
            "damage": "Treats gamma_BI prediction from FIRAS inversion as DERIVED.",
            "v2_0_action": "Reframe as consistency diagnostic, not derivation of gamma_BI from FIRAS."
        },
        {
            "source": "Paper 32 v1.5 extracted text",
            "paragraphs": [764, 765, 766],
            "damage": "R1-R4 / one-e-fold normalization language implies optical R4 forced by modular stack.",
            "v2_0_action": "Separate R1-R3/gauge payload from FIRAS-fixed optical R4."
        }
    ]
