...
This report is retained as the v1.5 framework-closure audit artifact. Its
`P4/R4` wording is superseded by the v2.0 R4/FIRAS audit in:

```text
reports/paper32_v20_r4_firas_kappa_audit_report.md
```

For Paper 32 v2.0, read every `P4/R4` closure phrase in this historical report
as **P4 source-block closure only**. Paper 32 KB7 does not derive Paper 17's
optical thermal `R4`. Optical `R4` is fixed by Paper 17 v1.5's FIRAS uniqueness
theorem.
...
4. classify each field using the canonical Claims Discipline labels, with scoped closures expressed as `DERIVED/CONDITIONAL_VERIFIED`, and with any fitted or hidden scalar flagged explicitly.
...
| Length scale `r_s` | Replace by `R_U`, `R`, or free length `L` | The IVP is on the physical hidden-support Schwarzschild variable. `r_s` is inherited from `M_U`; using another scale changes the support geometry. | `DERIVED/CONDITIONAL_VERIFIED` |
| Lambda dropout | Keep `Lambda_eff(R)` in acceleration | Dropout follows because the Paper 1/3 torsion-Lambda term scales as `1/R^2` and contributes a constant to `Rdot^2`; derivative with respect to `R` removes it. | `DERIVED/CONDITIONAL_VERIFIED` |
| Clamp evaluation `R=r_s` | Replace clamp radius by free `R_c` | `R=r_s, Rdot=0` is the support-clamp boundary condition. A different clamp radius is a different post-boundary model. | `DERIVED/CONDITIONAL_VERIFIED` |
...
| Candidate | Field-redefinition test | Rigidity result | Classification |
|---|---|---|---|
| Exponent `-1/4` | Replace by `-p` | The reduced observer visibility chain uses `Delta = Q x^4`; the boundary is `Delta=1`, so `x=Q^(-1/4)`. | `DERIVED/CONDITIONAL_VERIFIED` |
| Scalar `Q` | Replace by `f_Gamma`, `K_gauge`, or a free scalar | The termination equation is multiplicative in `Delta`, not logarithmic in `K_gauge`. `f_Gamma = Q^-1` is equivalent only if the exponent is transformed consistently. | `DERIVED/CONDITIONAL_VERIFIED` |
| Boundary condition `Delta=1` | Replace by threshold `Delta=tau` | The observer-cosmology termination artifact defines admissibility by nonnegative visibility; zero visibility occurs at `Delta=1`. Other thresholds are different observer-readout definitions. | `DERIVED/CONDITIONAL_VERIFIED` |
...
No hidden parameter. The correct status is `DERIVED/CONDITIONAL_VERIFIED` on the active reduced scalar optical-history observer class.
...
| Factor of 2 for full cycle | Replace by free multiplier | Forced only after symmetric application of the Paper 1/Poplawski bounce law on the collapsing branch. | `DERIVED/CONDITIONAL_VERIFIED` |
| Finite bounce attachment | Move bounce radius | `R_bounce` is computed from the imported high-density criterion; the attachment is conditional on that high-density EC/Poplawski criterion. | `DERIVED/CONDITIONAL_VERIFIED` |
| "Hard restart preferred" | Swap to soft restart | Raw stack does not force hard restart. Hard restart follows only under semantic restoration plus clock compatibility plus selected seed optical branch; soft restart survives under clamp-continuity philosophy. | `DERIVED/CONDITIONAL_VERIFIED` |
| Selector values `beta=1`, `lambda=1` | Replace by free positive scalars | Moment/recurrence selectors force `beta=lambda=1` only within a chosen seed optical family. The seed branch itself is not raw-stack unique. | `DERIVED/CONDITIONAL_VERIFIED` |
...
P4 source-block fixed-point normalization closed by DtN open-transport placement.
```
...
| Candidate | Field-redefinition test | Rigidity result | Classification |
|---|---|---|---|
| Source block selection | Move P4 to thermal/drag/recombination block | Typed boundary-to-bulk theorem and source-class lock restrict P4 to the active source/readout block. It does not automatically apply to thermal GTTP or drag-epoch observables. | `DERIVED/CONDITIONAL_VERIFIED` |
| DtN operator-valued transport placement | Replace `A_DtN = Lambda_src dr` by arbitrary semigroup representation | Later P4 repair artifacts promote the active reduced source map through DtN-type transport. Earlier KB.7 validation flagged a categorical extension risk; the repaired status is clean only on the active reduced source block. | `DERIVED/CONDITIONAL_VERIFIED` |
| Alpha class `alpha=1` | Replace by `alpha=2` | Paper 32 P4 alpha-class repair theorem fixes line/one-form class by primitive observable type, not operator order. | `DERIVED/CONDITIONAL_VERIFIED` |
| Local payload `Q` | Replace by `Q^p`, determinant power, or free scalar | Local payload lock theorem fixes the unique one-dimensional scalar source channel to multiplication by `Q=1+gamma^2`. | `DERIVED/CONDITIONAL_VERIFIED` |
...
The post-repair Paper 32 artifacts support KB.7 as `DERIVED/CONDITIONAL_VERIFIED` on the full active reduced source block. They do **not** support a universal P4 theorem for arbitrary unreduced observables, thermal GTTP, drag-epoch temperature maps, or every future typed block.
...
```text
DERIVED/CONDITIONAL_VERIFIED on the active reduced scalar source block.
Not a universal P4 closure for thermal, drag-epoch, or arbitrary unreduced observables.
Not a derivation of Paper 17 optical thermal R4.
```
...
| Candidate | Field-redefinition test | Rigidity result | Classification |
|---|---|---|---|
| Bridge-sector boundary | Move recombination/reionization/solver blocks into GMP | Typed boundary-to-bulk theorem separates bridge-placement observables from non-bridge history/solver sectors. | `DERIVED/CONDITIONAL_VERIFIED` |
| Non-bridge "inapplicable" classification | Treat as definitional dodge | It is structural: history/solver blocks have no puncture-load-to-spatial-channel placement predicate. GMP is not false there; it is not the applicable predicate. | `DERIVED/CONDITIONAL_VERIFIED` |
...
|---|---|---|
| SP-1 / Boundary Fixed-Point Principle | Retired by KB.7/P4 on active source block | Valid as `DERIVED/CONDITIONAL_VERIFIED` on active source block. Not universal P4. |
| SP-2 / Bridge state / H1 | Retired on bridge-readable quotient | Valid as `DERIVED/CONDITIONAL_VERIFIED` on quotient. Full ambient bridge extension uniqueness is not proved. |
| SP-3 / C2q Hawking state | Retired on lowest-shell quotient | Strongest of the four; clean on stated quotient. |
| SP-4 / GMP | Retired on realized typed bridge class | Valid as `DERIVED/CONDITIONAL_VERIFIED` on realized typed bridge class. Universal full-local-algebra GMP is false. |
...
|---|---|
| Power-State Readout / BFP | Closed as `DERIVED/CONDITIONAL_VERIFIED` on active source block via KB.7/P4. |
| Boundary covariance exponent | Closed as `DERIVED/CONDITIONAL_VERIFIED` inside the one-slot source transfer map. |
| Definitive `A_s` selection | Closed as `DERIVED/CONDITIONAL_VERIFIED` on lowest-shell quotient / source-block pipeline. |
...
|---|---|
| P4 source-block normalization | `DERIVED/CONDITIONAL_VERIFIED` on active source block; not universal. Optical R4 is FIRAS-fixed by Paper 17 v1.5, not derived by KB7. |
...
| `A_s` definitive | `DERIVED/CONDITIONAL_VERIFIED` on source/lowest-shell quotient. |
| `S8` tension | `DERIVED/CONDITIONAL_VERIFIED` on active Paper 10 legacy solver instantiation; later Paper 34 does not alter this. |
...
| Late-time bounce | `DERIVED/CONDITIONAL_VERIFIED` within extended fermionic/chiral IO-EC theory, plus conditional restart selectors. |
...
1. `KB.7/P4`: `DERIVED/CONDITIONAL_VERIFIED on the active reduced scalar source block`; not universal thermal/drag/P4.
...
4. `Late-time cycle`: `DERIVED/CONDITIONAL_VERIFIED` on symmetric Paper 1/Poplawski bounce attachment.
