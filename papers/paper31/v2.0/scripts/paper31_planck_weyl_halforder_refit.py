...
def main() -> None:
    ...
    results = {
        "constants": {"x": X, "lcmb_rescale": LCMB_RESCALE, "lcmb_tilt": LCMB_TILT},
        "start": START,
    ...
    lines = [
        "Paper 31 Planck Weyl Half-Order Refit",
        "=====================================",
        "",
        f"x = {X}",
        f"lcmb_rescale = x^(-1/2) = {LCMB_RESCALE:.15f}",
        f"lcmb_tilt = -1/2 = {LCMB_TILT:+.6f}",
    ...
    (OUT / "paper31_planck_weyl_halforder_refit_results.json").write_text(json.dumps(results, indent=2) + "\n")
    (OUT / "paper31_planck_weyl_halforder_refit_report.txt").write_text("\n".join(lines) + "\n")
