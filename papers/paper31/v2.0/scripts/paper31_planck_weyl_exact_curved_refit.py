...
BASE_PARAMS = {
    ...
    "lcmb_tilt": 0.0,
    "lcmb_pivot": 0.05,
    "lcmb_curved_shift": LCMB_CURVED_SHIFT,
    "lcmb_curved_order": LCMB_CURVED_ORDER,
}
...
def main() -> None:
    ...
    def get(a_s: float, tau: float) -> dict:
        key = (round(a_s, 15), round(tau, 12))
        if key not in cache:
            params = dict(BASE_PARAMS)
            params["A_s"] = a_s
            params["tau_reio"] = tau
            cache[key] = {"A_s": a_s, "tau": tau, **evaluate(params, plik)}
        return cache[key]
    ...
    for level, n in enumerate((7, 7, 9), start=1):
        ...
        best = {"A_s": level_best["A_s"], "tau_reio": level_best["tau"]}
        span_a *= 0.35
        span_tau *= 0.35
    ...
    lines = [
        "Paper 31 Planck Weyl Exact Curved Refit",
        "========================================",
        "",
        f"x = {X}",
        f"lcmb_rescale = x^(-1/2) = {LCMB_RESCALE:.15f}",
        f"lcmb_curved_shift = {LCMB_CURVED_SHIFT:+.6f}",
        f"lcmb_curved_order = {LCMB_CURVED_ORDER:+.6f}",
        "Kernel: x^(-1/2) * ((k^2 - 3K)/(k_p^2 - 3K))^(-1/4)",
    ...
    (OUT / "paper31_planck_weyl_exact_curved_refit_results.json").write_text(json.dumps(results, indent=2) + "\n")
    (OUT / "paper31_planck_weyl_exact_curved_refit_report.txt").write_text("\n".join(lines) + "\n")
