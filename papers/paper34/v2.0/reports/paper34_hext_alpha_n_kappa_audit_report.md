...
v2.0 note: this report is retained as the original H_ext(alpha,n) structural
audit. The separate v2.0 R4/FIRAS impact audit in
`paper34_v20_r4_firas_kappa_audit_report.md` verifies that the Paper 17 v1.5
R4 repair does not enter the Paper 34 H_ext scorecard. In particular,
`x^((n/2)K_gauge)` is the H0 stellar-photometric half-leg payload, not the
optical thermal readout `T_obs(R4)=T_IO*x^(R4*K_gauge)`.

**v2.0 supersession note.** The conditional-visibility warning in this original
field-redefinition audit has been addressed by the Paper 34 v2.0 §10 scoped
closure package. See `paper34_v2_0_section10_scoped_closure_memo.md`. The
numerical no-hidden-fit audit and anti-fit diagnostics remain unchanged.
...
Rigidity test: within Paper 34, the baseline is not chosen by fitting the six H0 measurements. It is inherited as the Paper 10 / Paper 29 projected observer branch. The formula then acts around that baseline.

v2.0 branch boundary: the Schur `68.91` package is a retired diagnostic branch after the Paper 29 hidden-parameter branch audit, not a live Paper 34 alternative. The current active registry has one Paper 34 baseline: `H0 = 67.57585653582628 km/s/Mpc` on the Paper 10 / Paper 29 projected observer branch.

Classification: `DERIVED/CONDITIONAL_VERIFIED` on the Paper 34 scorecard class; not fitted.

Hidden-parameter verdict: not hidden; high-leverage branch provenance is now explicit in Theorem 34.S10.1.
