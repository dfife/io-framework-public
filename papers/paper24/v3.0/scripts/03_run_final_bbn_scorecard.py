#!/usr/bin/env python3
"""
Paper 24 v3.0 reproducibility script 03.

Purpose:
    Recompute the excited-branch kernel calculation and the PRyMordial BBN
    scorecard used by Paper 24 v3.0. This script is the only PRyMordial-
    dependent script in the bundle.

    Paper 24 v3.0 inherits the Paper 17 v1.5 R4/FIRAS repair. The observed
    CMB temperature is not treated as an independent IO prediction. FIRAS fixes
    the observer-side optical readout normalization R4_FIRAS. That readout is
    recorded below for audit visibility, but the active BBN network calculation
    remains on the local interior thermal branch T_IO = 2.6635 K. Inserting the
    observer-side readout temperature into PRyMordial would mix observable
    classes and would not reproduce the Paper 24 BBN claim.
...
Claim boundary:
    ...
    - DERIVED/CONDITIONAL_VERIFIED: the branch-dressing model, with dependency on
      the Paper 22/Paper 24 bridge premises and imported nuclear inputs.
    - IMPORTED/EMPIRICAL: FIRAS fixes R4_FIRAS, inherited from Paper 17 v1.5.
...
if not PRYM_ROOT_ENV:
    raise SystemExit(
        "PRYM_ROOT is required for the full Paper 24 v3.0 PRyMordial rerun. "
...
X_IO = 1.519
T_OBS_FIRAS_READOUT_K = T_IO_BBN_BRANCH_K * (X_IO ** (R4_FIRAS * K_GAUGE))
...
def apply_baseline() -> None:
    """Configure PRyMordial to the Paper 24/Paper 22 BBN branch convention."""
    ...
    # BBN abundance/rate observables are evaluated on the local interior
    # thermal branch. R4_FIRAS fixes the observer-side optical readout and is
    # intentionally not inserted here.
    PRyMini.T0CMB = T_IO_BBN_BRANCH_K * PRyMini.Kelvin
...
def main() -> None:
    ...
    # weighted value is the branch object used by the v3.0 support chain.
    ...
    results = {
        "status": {
            "direct_rank2_position_route_for_excited_branch": "DERIVED/NO-GO",
            "direct_rank2_momentum_route_for_excited_branch": "DERIVED/NO-GO same tensor selection",
            "universal_linear_kinematic_shift_from_isotropic_TT_average": "DERIVED/NO-GO",
            "off_diagonal_mixing_alone_closes_gap": "DERIVED/NO-GO insufficient magnitude",
            "amplitude_weighted_external_capture_tail_closes_gap": "DERIVED/CONDITIONAL_VERIFIED",
        },
        "constants": {
            ...
            "temperature_branch_note": (
                "Paper 24 v3.0 uses T_IO for the PRyMordial BBN branch. "
                "R4_FIRAS fixes observer-side optical readout only."
            ),
            "epsilon_n": epsilon_n,
            "x": x,
            "Q_d": Q_d,
    ...
    report_lines = [
        ...
        "- DERIVED/NO-GO: off-diagonal p3/2<->p1/2 mixing is real but too small to close the gap by itself.",
        "- DERIVED/CONDITIONAL_VERIFIED: the amplitude-weighted external-capture tail of the weaker-bound 1/2- branch crosses the 1 sigma threshold under the stated bridge and nuclear-input package.",
        "",
    ...
    report_path = REPORTS_DIR / "final_excited_branch_report.txt"
