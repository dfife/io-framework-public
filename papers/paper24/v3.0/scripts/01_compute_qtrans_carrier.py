#!/usr/bin/env python3
"""
Paper 24 v3.0 reproducibility script 01.
...
Claim boundary:
    ...
    - DERIVED/CONDITIONAL_VERIFIED: the final-state measured-Q(7Li) carrier
      rule, with dependency on the imported nuclear-data bridge premise.
...
def compute() -> dict:
    """Return the Paper 24 v3.0 Q_trans carrier audit payload."""

    # IO and BBN constants used by the Paper 24 v3.0 carrier calculation.
    x = 1.519
    gamma = 0.2375
    q_d_barn = 0.0028578
    ...
    # Paper 24 v3.0 reports the carrier scale before the x^{-1} projection.
    q_trans_required_exact = x * q_req_old_barn
    return {
        "claim_discipline": {
            "Qtrans_from_measured_low_energy_E2_Sfactor": "DERIVED/NO-GO",
            "Qtrans_from_published_ab_initio_E2_capture_amplitudes": "DERIVED/NO-GO",
            "Qtrans_from_final_state_measured_Q7Li": "DERIVED/CONDITIONAL_VERIFIED",
            "Qtrans_from_minimal_cluster_model": "RECONSTRUCTION with imported nuclear data",
            "sqrt_minus_g_linear_TT_route_derives_Qtrans": "DERIVED/NO-GO",
        },
        "inputs": {
            "x": x,
            "gamma": gamma,
