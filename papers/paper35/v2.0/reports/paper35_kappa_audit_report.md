...
## v2.0 Supersession Note

This report is a historical v1.1/v1.2 forensic audit artifact. Its DESI target
17 entry predates the Paper 35 v2.0 RCA that restored the Paper 29 scoped BAO
readout kernel as the primary active IO DESI comparison. The raw
`chi2 = 69.484809` value remains archived only as the no-readout diagnostic in
`results/desi_confrontation_results.json`; the live v2.0 DESI comparison is the
scoped BAO readout result `chi2 = 27.735112876` with `dof = 13`.

## v1.2 R4/FIRAS Note

This report is the original Paper 35 v1.1 kappa audit artifact and is retained
for audit trail continuity. Paper 35 v1.2 supersedes the report's v1.1
late-time eta, temperature-conversion, and external leptogenesis target numbers
where they used the retired implicit optical readout normalization `R4 = 1`.
The active v1.2 values are in
`paper35_v12_r4_firas_kappa_audit_report.md`,
`paper35_v12_r4_firas_kappa_audit_results.json`, and the v1.2 scripts/results.
The theorem-surface classifications and registry counts are unchanged.
...
Historical manuscript claim: old observer-side `w = -1/3` is retired; active
observer-side vacuum is `w = -1`; raw no-readout active branch gives
`chi2 = 69.48`; IO does not predict the DESI evolving-dark-energy signal.
...
- raw no-readout active branch public DR2 GCcomb diagnostic:
  `chi2 = 69.484809`.
- v2.0 live scoped BAO readout comparison:
  `chi2 = 27.735112876`, fixed flat LambdaCDM comparator
  `chi2 = 29.869750531`, `Delta chi2 = -2.134637654`.
...
Recommendation: Preserve the correction framing, but do not cite the raw
`chi2 = 69.48` no-readout branch as the live Paper 35 v2.0 DESI claim. Cite
script 07's scoped BAO readout output for the primary active comparison.
