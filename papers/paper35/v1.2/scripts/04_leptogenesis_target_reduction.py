#!/usr/bin/env python3
"""Paper 35 v1.2 script 04: leptogenesis target reduction.

Purpose:
    Convert the v1.2 FIRAS-normalized IO eta target into standard external leptogenesis target
...
Claim boundary:
    ...
    target reduction, not IO source-sector closure. The input eta_late inherits
    the Paper 17 v1.5 FIRAS-fixed observer-side thermal readout.
"""
...
def main() -> int:
    ...
    # Paper 35 carries two nearby but conceptually distinct scales:
    #
    # 1. The chiral source-era diagnostic T_f from script 03.  That value is a
    #    K_gauge^4 diagnostic and does not move under the R4/FIRAS repair.
    # 2. The Poplawski target-compatibility scale sqrt(Y_B) * Mbar_Pl used in
    #    the external leptogenesis target-reduction slice.  Because Y_B is
    #    inherited from eta_late, this scale moves with the v1.2 FIRAS-fixed
    #    eta target.  Keeping both values visible prevents a hidden stale
    #    hard-coded v1.1 target.
    t_f_chiral_diagnostic_gev = 2.20039385974778e13
    poplawski_reduced_mass_scale_gev = 2.435e18
    t_f_poplawski_eta_target_gev = math.sqrt(y_b) * poplawski_reduced_mass_scale_gev
    epsilon_max_at_t_f = c_di * t_f_poplawski_eta_target_gev
    ...
    payload = {
        ...
        "formulae": {
            ...
            "T_f_poplawski_eta_target": "sqrt(Y_B) Mbar_Pl,reduced",
        },
        "numbers": {
            "eta_late": eta_late,
            "Y_B_target": y_b,
            ...
            "T_f_target_GeV": t_f_poplawski_eta_target_gev,
            "T_f_chiral_diagnostic_GeV": t_f_chiral_diagnostic_gev,
            "T_f_poplawski_eta_target_GeV": t_f_poplawski_eta_target_gev,
            "Poplawski_reduced_mass_scale_GeV": poplawski_reduced_mass_scale_gev,
            "epsilon_max_at_T_f": epsilon_max_at_t_f,
            "kappa_required_if_M1_equals_T_f": kappa_required_if_m1_equals_t_f,
            "T_f_over_M1_floor_absolute": t_f_poplawski_eta_target_gev / m1_floor_absolute,
        },
    }
