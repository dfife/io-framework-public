#!/usr/bin/env python3
"""Validate every frozen Paper 25 v2.0 bundle output.
...
def main() -> int:
    ...
    record(checks, "theorem_count", len(ledger["theorems"]) == 13)
    ...
    record(checks, "active_D_over_H", close(active["D_over_H"], 2.5072097840055007e-05, 1.0e-18))
    record(checks, "active_D_over_H_sigma", close(active["D_over_H_sigma"], -0.659673866483311, 1.0e-15))
    record(checks, "active_Y_p", close(active["Y_p"], 0.24770877182909237, 1.0e-15))
    record(checks, "active_Y_p_sigma", close(active["Y_p_sigma"], 0.6771929572730941, 1.0e-15))
    record(checks, "active_Li7", close(active["Li7_over_H"], 1.7414708079857392e-10, 1.0e-22))
    record(checks, "active_Li7_sigma", close(active["Li7_over_H_sigma"], 0.520873574147546, 1.0e-15))
    record(checks, "active_chi2", close(active["chi2_3obs"], 1.1650691917465592, 1.0e-15))
    record(checks, "active_R_34_tot", close(active["R_34_tot_T9eff"], 0.31003925909097196, 1.0e-15))
    ...
    record(checks, "R4_FIRAS_recorded", close(audit["R4_FIRAS"], 1.0031014644, 1.0e-12))
